...
This template allows you to deploy a simple VM Scale Set of Linux VMs (Standard_F16) using the latest patched version of Ubuntu Linux 16.04.0-LTS, 15.10 or 14.04.4-LTS. This template also deploys a jumpbox with a public IP address in the same virtual network. You can connect to the jumpbox via this public IP address, then connect from there to VMs in the scale set via private IP addresses. To ssh into the jumpbox, you could use the following command:
